...
import dev.tesserakt.sparql.types.runtime.query.Query.ResultChange.Companion.into
import kotlin.jvm.JvmInline

sealed class Query<ResultType, Q: Query>(
    protected val ast: Q
) {

    internal inner class Processor {

        private val state = BasicGraphPattern(ast = ast.body)
        ...
        fun state(): List<ResultType> {
            return state
                ...
                .map { bindings -> this@Query.process(ResultChange.New(bindings.value)).value }
        }
