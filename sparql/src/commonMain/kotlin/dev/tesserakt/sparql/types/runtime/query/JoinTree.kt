...
import dev.tesserakt.sparql.types.runtime.*
...
import dev.tesserakt.sparql.types.runtime.evaluation.mapToStream
import dev.tesserakt.sparql.types.runtime.query.TriplePattern.Companion.createIncrementalPatternState
import dev.tesserakt.sparql.types.runtime.stream.*
import dev.tesserakt.sparql.types.util.Bitmask
...
internal sealed interface JoinTree: MutableJoinState {
    ...
    data object Empty: JoinTree {
        ...
        override fun peek(delta: _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.DataDelta): OptimisedStream<MappingDelta> {
            return emptyStream()
        }

        override fun process(delta: _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.DataDelta) {
            // nothing to do
        }
    ...
    value class None<J: MutableJoinState>(private val states: List<J>): JoinTree {
        ...
        fun join(completed: Bitmask, delta: MappingDelta): List<MappingDelta> {
            ...
            return when (delta) {
                is _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.MappingAddition -> {
                ...
                is _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.MappingDeletion -> {
                    var results: List<MappingDelta> = listOf(delta)
                    completed.inv().forEach { i ->
                        results = results.flatMap { states[i].join(it) }
                        if (results.isEmpty()) {
        ...
        companion object {
            ...
            operator fun invoke(unions: List<Union>) = None(
                states = unions.map { dev.tesserakt.sparql.types.runtime.query.Union(it) }
            )
    ...
    value class Dynamic<J: MutableJoinState> private constructor(private val root: Node<J>): JoinTree {
        ...
        sealed interface Node<J: MutableJoinState> {
             ...
             */
            fun peek(delta: _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.DataDelta): OptimisedStream<MappingDelta>

            /**
             * Processes the [delta], updating the node accordingly
             */
            fun process(delta: _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.DataDelta)
            ...
            class Connected<J: MutableJoinState, L: Node<J>, R: Node<J>>(
                ...
                override fun process(delta: _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.DataDelta) {
                    peek(delta).forEach { diff ->
                        when (diff) {
                            is _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.MappingAddition -> buf.add(diff.value)
                            is _root_ide_package_.dev.tesserakt.sparql.types.runtime.evaluation.MappingDeletion -> buf.remove(diff.value)
                        }
                    }
                    left.process(delta)
                    right.process(delta)
        ...
        companion object {
            ...
            @JvmName("forPatterns")
            operator fun invoke(patterns: List<Pattern>): Dynamic<TriplePattern<*>> {
            ...
            @JvmName("forUnions")
            operator fun invoke(unions: List<Union>): Dynamic<dev.tesserakt.sparql.types.runtime.query.Union> {
                val states = unions.map { dev.tesserakt.sparql.types.runtime.query.Union(it) }
                val root = build(states)
                return Dynamic(root)
            }
