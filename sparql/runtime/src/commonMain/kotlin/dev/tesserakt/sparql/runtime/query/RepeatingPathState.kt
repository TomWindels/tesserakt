...
import dev.tesserakt.sparql.util.SegmentsList
        ...
        override fun process(delta: DataDelta): OptimisedStream<MappingDelta> {
            ...
            return when (delta) {
                is DataAddition -> {
                    ...
                    arr.addAll(result.toStream().mapped { it.inner })
                ...
                is DataDeletion -> {
                    ...
                    arr.removeAll(result.toStream().mapped { it.inner })
...
//                    val new = inner.process(delta)
//                        .map { SegmentsList.Segment(start = it.value.get(start.id)!!, end = it.value.get(end.id)!!) }
//                    segments.insert(new)
//                    terms.increment(TermIdentifier(quad.s))
//                    terms.increment(TermIdentifier(quad.o))
//                }
...
//                    val removed = inner.process(delta)
//                        .map { SegmentsList.Segment(start = it.value.get(start.id)!!, end = it.value.get(end.id)!!) }
...
//    }
    ...
    abstract fun process(delta: DataDelta): Stream<MappingDelta>
