...
class Bitmask private constructor(
    ...
    infix fun or(other: Bitmask): Bitmask =
        Bitmask(
            bits = this.bits or other.bits,
            length = maxOf(this.length, other.length)
        )

    infix fun and(other: Bitmask): Bitmask =
        Bitmask(
    ...
    companion object {
        ...
        fun wrap(raw: Int, length: Int): Bitmask =
            Bitmask(bits = raw, length = length)
    ...
    fun withOnesAt(vararg index: Int): Bitmask {
        return Bitmask(
            bits = index.fold(bits) { result, i -> result or (1 shl i) },
            length = length
        )
    }
