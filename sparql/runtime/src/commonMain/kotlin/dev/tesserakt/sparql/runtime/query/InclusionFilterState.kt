        ...
        init {
            ...
            filter
                ...
                .forEach { mappingDelta ->
                    when (mappingDelta) {
                        is MappingAddition -> filtered.increment(
                            mappingDelta.value.retain(commonBindingNames).hashable()
                        )
                        // highly unlikely occurrence considering we're joining on an empty mapping
                        is MappingDeletion -> filtered.decrement(
                            mappingDelta.value.retain(commonBindingNames).hashable()
                        )
                    }
                }
        ...
        override fun process(): OptimisedStream<MappingDelta> {
            ...
            val innerChanges = inner
                ...
                .filtered { delta ->
                    ...
                    val current = filtered[hashed]
                    if (current == 0) {
                        return@filtered false
                    }

                    // could be filtered out now
                    filterChanges[hashed].let { it == null || it + current > 0 }
                }
