...
sealed interface OptionalState {
    ...
    class AppliedOptionalMultiple(
        ...
        override val cardinality: Cardinality
            get() = inner.cardinality * optionals
                .fold(OneCardinality) { cardinality, optional ->
                    cardinality * optional.state.cardinality.coerceAtLeast(OneCardinality)
                }
        ...
        override fun peek(delta: DataDelta): OptimisedStream<MappingDelta> {
            ...
            val affectsOptionals = optionals.any { it.peek(delta) != OptionalBlock.PeekResult.UNCHANGED }
            return if (!affectsOptionals) {
                optionals.fold(inner.peek(delta)) { stream, optional ->
                    optional.optionalJoin(stream).optimisedForSingleUse()
                }
            } else {
    ...
    companion object {
        ...
        operator fun invoke(
            ...
            var parentBindings = inner.bindings
            val optionals = optionals.map { optional ->
                OptionalBlock(
                    ...
                    .also { parentBindings += it.state.bindings }
            }
